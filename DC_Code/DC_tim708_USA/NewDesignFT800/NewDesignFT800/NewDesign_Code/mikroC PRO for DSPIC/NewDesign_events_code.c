...
float CalculateSpeed(unsigned char *tBuff, unsigned char *encBuff){//M/S
      ...
      speedtemp=(newPos-oldPos)/cycle*1.2267;  //distance traveled in meters= ticks/(ticks/cycle)*(meters/cycle)
      speedtemp=speedtemp/((newT-oldT)/1000.0);
      oldT=newT;
      oldPos=newPos;
      //speedCounter++;
     // averageSpeed+=speedtemp;  //average speed used for pic trigger counter 12/14/2014 Tim
      return speedtemp;
}
...
void EveButtonProfStartProfilingOnClick() {
  ...
  //  Initplot(ProfilingScreen_Lines+25,25,34,172,8,&incplotflag);
    ...
    while(laserplotflag){
     ...
     //sprintf(EveTextSUAverageSpeed.Caption, "%u",averageSpeed);
